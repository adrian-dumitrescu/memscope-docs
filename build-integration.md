...
firmware_footprint_report(
  ...
  TEXT     ${CMAKE_BINARY_DIR}/reports/app-layout.txt   # full-memory hierarchical text dump
  MARKDOWN ${CMAKE_BINARY_DIR}/reports/app-layout.md    # same content, markdown-formatted
  TOP_SYMBOLS 0                                         # 0 = show all symbols, no cap
  STRICT
)
```
...
The `TEXT` and `MARKDOWN` outputs are full-memory hierarchical dumps designed for
PR comments and `diff`-friendly snapshots — see
[CLI reference → Memory layout dumps](cli-reference.md#memory-layout-dumps).
A common CI pattern is to attach the `.md` artifact as a sticky PR comment so
every commit shows the complete memory layout for review.
