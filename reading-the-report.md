...
7. `diagnostics`: assumptions, unresolved ambiguities, policy trace, telemetry mode.
...
When you produce a combined report from two or more cores — either via repeated
`--core NAME=ELF[,MAP[,LINKER]]` flags
([cli-reference.md § Multi-core analysis](cli-reference.md#multi-core-analysis))
or via `[[inputs.cores]]` in a TOML config
([configuration-reference.md § Multi-core analysis](configuration-reference.md#multi-core-analysis))
— the HTML report exposes three multicore-only surfaces. They are hidden in
single-image reports.

> **Heuristic detection.** MemScope can also infer multicore from a single
> linker script whose `MEMORY{}` block declares per-core regions like
> `core0_*`, `cm4_*`, or `int_flash_c0`. This is convenient on SoCs whose
> region naming naturally segregates by core — but it's a heuristic, and
> can misfire when a vendor linker script statically declares all per-core
> regions even on single-image builds (STM32H7 dual-core variants used
> single-core, etc.). If your report shows the multicore surfaces below
> on a build you consider single-image, pass `--no-infer-multicore` (or
> set `inputs.infer_multicore_from_region_names = false` in TOML) to
> require explicit `--core` / `[[inputs.cores]]` for multicore detection.

- **Group-by Core mode** — the Memory Layout Explorer's `Group by` popover
  shows a 4th pill (`Core`) alongside `Region`, `Object`, and `Zone (TOML)`.
  The pill is enabled only when the report has at least two distinct cores.
  Picking it reshapes the canvas to one row per core; each row's address
  axis spans the union of memory regions that core's image occupies, with
  per-region tick marks where one region's address range ends and the next
  begins. All three lens overlays (Alignment Lens, Padding Heatmap, Memory
  Roles) keep working unchanged. The searchable list in the popover lists
  the cores with their total bytes so you can jump straight to one.
- **Per-core color chips** — every range that's attributed to a core
  carries a stable color chip showing the core's id. The chip appears in
  the hover tooltip's `Core:` line, in the side panel's `Selection` block
  next to `Core`, and on the headers of the Per-Core Utilization cards
  (described below). The same `core_id` always maps to the same color
  across every surface and across runs — derived from a hash of the id
  against a fixed 6-color palette — so you can disambiguate a range's
  owner at a glance without re-reading the label.
- **Per-Core Utilization cards** — the Visual Insights section gets a
  `Per-Core Utilization` card containing one sub-card per core. Each
  sub-card shows Flash and RAM usage as the familiar progress bars
  (yellow at 85%, red at 100%, same thresholds as Region Utilization) plus
  the top 3 size-contributing sections attributed to that core. The
  capacity denominator is the sum of capacities of regions the core's
  sections live in; for *shared* regions (no per-core split) the full
  region capacity counts toward every core touching it, so percentages on
  shared regions read slightly higher than the per-core "fair share" —
  the absolute used/capacity labels make the basis explicit.

At the JSON level, every section, symbol, and range carries a `core_id`
field (or `null` for single-image runs), and a top-level `cores` summary
lists the distinct cores with their total bytes and the regions they
touch — same data the HTML surfaces above are built from.
