...
csv      = "reports/app.csv"
text     = "reports/app-layout.txt"     # full-memory hierarchical text dump
markdown = "reports/app-layout.md"      # same content, markdown-formatted
layout_top_symbols = 5                  # 0 = show every symbol per section, no cap
...
Same path-override rule as `[inputs]`: CLI `--json`/`--html`/`--csv`/`--text`/`--markdown` wins. The CLI `--top-symbols N` flag is the matching override for `layout_top_symbols`.

`text` and `markdown` produce a hierarchical full-memory dump (every region →
every section in address order → top 5 symbols per section, with sizes /
addresses / percentages / owning object). Designed for PR comments, build logs,
and `diff`-friendly snapshots that JSON and CSV can't provide. See [CLI reference
→ Memory layout dumps](cli-reference.md#memory-layout-dumps) for sample output.
...
Off by default. Both keys are required to turn it on: `enabled = true`
without an `endpoint` is a **configuration error** and MemScope refuses
to load the config rather than guessing a destination.
...
When enabled, MemScope POSTs exactly three fields — `version`,
`command`, `runtime_ms` — to the endpoint **you** specify, after the
command completes. Best-effort with a 1-second timeout; network errors
and non-2xx responses are dropped silently and never change the exit
code. A whitelist in code prevents any other field from being
transmitted. The Licensor neither receives nor stores this data.
